...
class AStar_Solver:
    ...
    def Solve(self):
        ...
        while(not self.path and self.priorityQueue.qsize()):
            closestChild = self.priorityQueue.get()[2]   # startState from the tuple
            ...
            for child in closestChild.children:
                if child.value not in self.visitedQueue:
                    count += 1
                    if not child.dist:
                        self.path = child.path   # found the solution, break out of for loop and then out of while loop
                        break
